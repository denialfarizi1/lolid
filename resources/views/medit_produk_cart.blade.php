...
@section('content')
  ...
  <h3>  KERANJANG BELANJA [ <small> {{$count_cart}} Item(s) </small>]<a href="produk=m" class="btn btn-medium pull-right"><i class="icon-arrow-left"></i> Continue Shopping </a></h3>  
  <hr class="soft"/>
  @guest
  ...
  @if(!empty($cart))
                     @foreach($cart as $c)
                     <form action="{{route('update_cart_produk')}}" method="POST">
                    {{ csrf_field() }}
  <table class="table table-bordered">
              <thead>
                <tr>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <form action="/cart_produk/update" method="POST">
                     {{ csrf_field() }} 
                  <td>  <input type="hidden" name="cart_id" value="{{$c->cart_id}}">
                        <input type="hidden" name="asal" value="mcart_produk"> 
                        <input type="number" name="produk_cart_qty" id="harga_algoritma" value="{{$c->produk_cart_qty}}" style="width: 30px" onchange="total()">
                  ...
                  <td>
                    <div style="width: 130px;">
                      ...
                      <a href="mproduk_{{$c->produk_id}}"><button type="button" class="btn btn-info btn-mini" > detail</button></a>
                     </div>
                      <div style="width: 40px; float: left;">
