  ...
  <div class="row">
   <center>   
      <div id="gallery" class="span3" style="width: 350px; border: 0px solid;">
            <a href="{{'image/view/'.$produk_user->produk_image_src1}}" title="{{$produk_user->produk_name}}">
        <img src="{{url('image/view/'.$produk_user->produk_image_src1)}}" style="width:100%" alt="{{$produk_user->produk_name}}"/>
            </a>
      ...
      <div class="span6">
        ...
        <h6><img src="themes/images/alamat.png" style="width: 17px; float: left; padding-right: 3px;"><div style="float: left;">{{$produk_user->produk_lokasi}}</div></h6>
        </div>
        <hr class="soft"/>
        ...
        @else
         @endguest  
          ...
          <label class="control-label" style="text-align: left; font-size: 20px;"><span class="btn btn-medium btn-primary">Rp.{{$produk_user->produk_price}},00</span></label>
              <div class="controls">
                  @guest
        ...
        <hr class="soft"  />
        <div class="clr" style="text-align: left; padding-top: 20px; height: 80px; font-size: 20px;">[{{$produk_user->produk_qty}}] barang tersedia
        </div>
      ...
      <div class="user" style="width: 300px ; height: 200px;  border: 0px solid; padding-left: 200px; font-size: 15px;">
        ...
        <table>
          <tr>
            ...
            <td>
              ...
              <a href="{{'@'.$produk_user->get_user->username}}">
                ...
                <img src="{{url('image_user/view/'.$produk_user->get_user->image_profil)}}" class="img-circle" style="width: 40px; height: 40px;  padding: 5px;">
                  @else
          ...
          <tr>
            ...
            <td style="padding-left: 10px;">:</td>
            <td style="padding-left: 10px;">0{{ $produk_user->get_user->hp }}</td>
          </tr>
          <tr>
    ...
    <div class="row">   
      <div class="span2">
        <a href="produk_{{$p->user_id}}"><center><img src="{{url('image/view/'.$p->produk_image_src1)}}" alt="" style="width: auto; height: 110px;" /></center></a>
      </div>
      <div class="span4">
        <center>
        <h3>{{$p->produk_name}}</h3>
      ...
      <h2 class="btn btn-mini btn-primary"> Rp.{{$p->produk_price}},00</h2>
      ...
      <center>
        ...
        <form action="register" method="post" enctype="multipart/form-data">
                ...
                <a href="mproduk_{{$p->produk_id}}" class="btn btn-mini"><i class="icon-zoom-in"></i></a>
         ...
         @else
          <form action="{{route('cart_save')}}" method="post" enctype="multipart/form-data">
                ...
                <input type="hidden" name="user_id" id="" value="{{Auth::user()->id}}"/>
                <input type="hidden" name="produk_id" id="" value="{{$p->produk_id}}"/>           
                <input type="hidden" name="produk_name" id="" value="{{$p->produk_name}}">
                <input type="hidden" name="produk_price" id="" value="{{$p->produk_price}}"/>
                <input type="hidden" name="produk_image_src1" id="" value="{{$p->produk_image_src1}}"/>
                <input type="hidden" name="produk_cart_qty" class="span1" placeholder="Qty." value="1" />
       ...
       @if(!empty($produk))
        ...
        <div class="caption">
          ...
          <div style="padding-left: 10px;">
           <center><h5>{{$p->produk_name}}</h5></center>
          <div style="height: 20px;"> 
           <center>{{$p->produk_model}}</center>
          </div>
          
         </div>
           <h4 style="text-align:center"><a class="btn btn-primary btn-mini" href="#">Rp.{{$p->produk_price}},00</a>
           </h4>
        ...
        <form action="register" method="post" enctype="multipart/form-data">
                ...
                <center>
                <a href="register" class="btn btn-medium btn-primary">Add to <i class=" icon-shopping-cart"></i></a>
                <a href="mproduk_{{$p->produk_id}}" class="btn btn-medium"><i class="icon-zoom-in"></i></a>
                </center>
        </form>
         @else
          <form action="{{route('cart_save')}}" method="post" enctype="multipart/form-data">
  ...
  <div style="width: 500px; height: auto; border: 1px ; padding-top: 25px;">
    ...
    <div style="width: 480px; height: 45px; border: 1px ;  padding-top: 5px; padding-left: 30px">
      <form action="{{route('comment_produk_save')}}"" method="POST">
        ...
        <input type="hidden" name="asal" value="{{'mproduk_'.$produk_user->produk_id}}">
